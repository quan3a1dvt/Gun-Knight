...
Imp::Imp()
{
	...
	m_spawnState = new Imp_SSpawn(this);
	m_idleState = new Imp_SIdle(this);
	m_runState = new Imp_SRun(this);
	m_attackState = new Imp_SAttack(this);
	m_deathState = new Imp_SDeath(this);
	
	
	m_HitBox = new HitBox(sf::Vector2i(11 * m_scale.x, 11 * m_scale.x));
	...
	m_AttackHitBox = new HitBox(sf::Vector2i(55 * m_scale.x, 55 * m_scale.x));
...
void Imp::ChangeNextState(IMState::STATE nextState)
{
...
void Imp::Update(float deltaTime)
{
	...
	if (m_Active) {
		...
		PerformStateChange();
		m_AttackHitBox->setPosition(m_HitBox->getPosition());
		if (m_currentState != m_spawnState) {
			...
			if (m_Hp.x <= 0) {
				m_canMove = false;
				if (m_currentState != m_attackState) ChangeNextState(IMState::STATE_DEATH);
			}
		...
		m_HitNumber.setCharacterSize(13.0f);
...
void Imp::Render(sf::RenderWindow* window)
{
	if (m_Active) {
...
void Imp::PerformAIMovement(float deltaTime)
{
	...
	if (m_choosedMoveWay == 0 && m_canMove) {
		...
		if (m_followTime >= m_waitTime) {
			...
			if (fabs(m_destination.x - m_HitBox->getPosition().x) >= randValue || fabs(m_destination.y - m_HitBox->getPosition().y) >= randValue && m_stop == false) {
				...
				m_waitTime = random::random(2, 5) * 1.f / 10.f;
					...
					if (m_currentState != m_attackState) {
						printf("da\n");
						GConnect->getPlayer()->LoseHp(15);
						ChangeNextState(IMState::STATE_ATTACK);
				}
			}
		...
		if (m_followTime < m_followTimeLimit) {
			if ((fabs(m_destination.x - m_HitBox->getPosition().x) >= randValue || fabs(m_destination.y - m_HitBox->getPosition().y) >= randValue) && m_stop == false) {
				...
				m_choosedMoveWay = 0;
				m_waitTime = random::random(2, 5) * 1.f / 10.f;
				if (Collision::BoundingBoxTest(m_HitBox, GConnect->getPlayer()->getHitBox())) {
...
void Imp::Move(float movex, float movey)
{
...
bool Imp::getShowShader()
{
	return m_ShowShader;
}

Animation* Imp::getSprite()
{
	return m_currentState->getSprite();
}

sf::Shader* Imp::getShader()
{
	return m_Shader;
}

void Imp::PerformStateChange()
{
